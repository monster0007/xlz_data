...
object XHCustomer {
    ...
    def main(args: Array[String]) {
      ...
      val url230="jdbc:mysql://10.1.24.230:3306/biz_xh?"
    ...
    //读取230百川的三张需要用到的表
      val customer=spark.read.jdbc(url230,"u_custom",prop)
    ...
    //208读取表的临时表
     // pcd_custom.createOrReplaceTempView("pcd_custom") 可拿可不拿
      ...
      //230读取表的临时表
     customer.createOrReplaceTempView("u_custom")

      spark.udf.register("concat",(str:String)=>{
        "2000".concat(str)
      })
      //读取230表客户的8个字段 insert into  pcd_custom_88537_20190508(id,customname,customtype,address,contactname,phone)
      //SELECT customid,customname,customtype,address,ordername,ordertel FROM `u_custom`;
      val aa=spark.sql("SELECT concat(customid) as id, customname,customtype,address,ordername as contactname,ordertel as phone from u_custom")
          aa.createOrReplaceTempView("xh_customer_source")
      aa.show()
      //挑出新增客户
    val notinpcd_customer=spark.sql("select * from xh_customer_source a where not exists (select b.customid from pcd_custom_relationship b where a.id=b.customid)")
         //notinpcd_customer.createOrReplaceTempView("bc_customer")
      notinpcd_customer.show()
      notinpcd_customer.write.mode("overwrite").jdbc(urllocal,"xh_customer_20190618",prop3)

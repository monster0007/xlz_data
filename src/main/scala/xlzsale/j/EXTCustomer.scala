        ...
        if (str1==null){
          ...
          if (str2.matches("^1000.+?")) {
           ...
           //   aff.createOrReplaceTempView("aff")
              ...
              val DF1=spark.sql("select * from rx_customer_source a where a.id not in(select b.customerid from pcd_custom_relationship b )")
              //读取鑫和客户的8个字段，挑出新增客户
              ...
              val DF2=spark.sql("select * from xh_customer_source a where a.id not in(select b.customerid from pcd_custom_relationship b )")
              //读取百川客户的8个字段，挑出新增客户
              ...
              val DF3=spark.sql("select * from bc_customer_source a where a.id not in(select b.customerid from pcd_custom_relationship b )")
              //读取康力客户的8个字段，挑出新增客户
              ...
              val DF4=spark.sql("select * from kl_customer_source a where a.id not in(select b.customerid from pcd_custom_relationship b )")
              //读取新绿洲客户的8个字段，挑出新增客户
              ...
              val DF5=spark.sql("select * from lzn_customer_source a where a.id not in (select b.customerid from pcd_custom_relationship b )")
              ...
              val DF6=spark.sql("select * from sj_customer_source a where a.id not in (select b.customerid from pcd_custom_relationship b )")
             //读取新阳客户的8个字段，挑出新增客户
              spark.sql("use hiveonhbase")
      val a7=spark.sql("select concat(7000,a.id) as id,a.name as customname,b.name as customtype,a.registeraddr as address,'' as province,'' as city,a.legalperson contactname,a.contact phone from xy_bs_customer a left join xy_bs_customclass b on a.customertype=b.id")
              a7.createOrReplaceTempView("xy_customer_source")
      val DF7=spark.sql("select * from xy_customer_source a where a.id not in (select b.customerid from pcd_custom_relationship b )")
...
//省全的
      ...
      DFALL.write.mode("overwrite").jdbc(urllocal,"pcd_new_customer_未处理总数",prop)
